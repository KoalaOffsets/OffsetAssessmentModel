...
for (t in 0:tSimul) {
  ...
  for (i in 1:length(luLabel)){
    ...
    idDummy.pl    <- macroVar$plan2017 %in% as.numeric(test$xlevels$plan2017fact)
    idDummy.sa    <- macroVar$sa4 %in% as.numeric(test$xlevels$sa4fact)
    idDummy       <- idDummy.pl & idDummy.sa
  ...
  ##____4.2.5 -- Filter transition probability 0, return to original land class  #### 
  ...
  for (i in 1:length(luLabel) ){
    ...
    idTP          <- which(tp.cover$luDummy == luLabel[i] & is.na(tp.cover[,id10]))
